...
//------------서버연동 ------------
function ConnectServer(nodejs_connector_url, l_company_id, l_userid, exten, passwd, first_status, from_ui) {
    userid = l_userid;
    ...
    try {
        ...
        alert("[socket_frame.js][ConnectServer][catch]서버가 정상인지 확인후 사용해주세요");
    }
}
...
function parseMessage(msg) {
    ...
    if(event == "LOGIN") {
        ...
        parent.parseLogin(kind,data1,data2,data3,data4,data5,data6,data7,data8,data9);
        return;
    } else if(event == "PEER") {
        ...
        parent.parseMemberStatus(kind);
		    ...
		    //내선일때 팝업을 막고 싶다면
        if((data1.length==3 && data2.length==3) || (data1.length==4 && data2.length==4)) {
            return;
        }
